"""
φ-Attention 长程信息保持力合成实验 v2
======================================
纯numpy forward pass，无需训练。
测量 phi5/std5/std8/phi8 在不同序列长度下对 position 0 注入信号的长程保持能力。

v2 改进：
  - 超强信号注入 (SIGNAL_STRENGTH=100, SIGNAL_DIM=32)，确保 K[0] 范数远大于其他位置
  - 更多灵敏指标：rank_of_pos0, top1_fraction, max_head_attn
  - 10个seed降低方差
  - 使用 d_model=160 节省内存
"""
...
DATA_PATH = os.path.join(SCRIPT_DIR, 'phi_longrange_data.json')
REPORT_PATH = os.path.join(SCRIPT_DIR, 'phi_longrange_report.md')
...
def compute_metrics(X: np.ndarray, output: np.ndarray, attn_weights: np.ndarray):
    ...
    # 1.0 = pos0排名第一, 0.0 = pos0排名最后
    rank_vals = []
    for b in range(B):
        for h in range(H):
            last_row = attn_weights[b, h, -1, :]  # (L,)
            rank = np.sum(last_row > last_row[0])  # 有多少位置比pos0权重大
            normalized_rank = 1.0 - rank / (L - 1)  # 归一化到[0,1]
...
def run_experiment():
    print("=" * 80)
    print("φ-Attention 长程信息保持力合成实验 v2")
    print("=" * 80)
        ...
        for L in SEQ_LENGTHS:
            ...
            for seed in range(N_SEEDS):
                ...
                if seed == 0 or seed == N_SEEDS - 1:
                    print(f"  [{model_name}] L={L:4d} seed={seed} | "
    ...
    for model_name in MODELS:
        ...
        for metric in METRICS:
            ...
            # 对于 rank_of_pos0 和 top1_fraction，正值指标（越高越好）
            # 对于 attn_to_pos0，是正的
            valid = means_arr > 0
            if valid.sum() >= 2:
    ...
    for metric in METRICS:
        print(f"\n--- {metric} (mean) ---")
        header = f"{'Model':<8}" + "".join(f"  L={L:<8}" for L in SEQ_LENGTHS)
        print(header)
        print("-" * len(header))
        for model_name in MODELS:
            row_vals = []
            for L in SEQ_LENGTHS:
                ...
                row_vals.append(f"{m:.5f}±{s:.5f}")
            print(f"{model_name:<8}" + "".join(f"  {v:<18}" for v in row_vals))

    # ============================================================
    # 生成报告
    # ============================================================
    generate_report(summary, decay_analysis, output_data, total_time)

    return output_data


def generate_report(summary, decay_analysis, output_data, total_time):
    lines = []
    lines.append("# φ-Attention 长程信息保持力合成实验报告")
    lines.append("")
    lines.append("## 实验设计")
    lines.append("")
    lines.append("### 目的")
    ...
    # 对 attn_to_pos0, info_retention, rank_of_pos0, top1_fraction, max_head_attn：斜率越大（越接近0）衰减越慢
    # 对 signal_corr：斜率越大越好
    # 统一：斜率越大 = 衰减越慢 = 越好
    best_model = {}
    for metric in METRICS:
    ...
    lines.append("理论上应产生更稳定的远端注意力保持。实验验证了这一预测的强度。")
    lines.append("")
