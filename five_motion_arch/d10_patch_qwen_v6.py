"""
D10五动管道架构改造脚本 v6
修复: 1) 从config预存d10_*属性，不依赖self.num_heads等
      2) tokenizer不返回position_ids时自动生成
      3) KV cache兼容transformers 5.x Cache对象
用法：python d10_patch_qwen_v6.py
"""
...
# --- 重写Attention forward ---
# ★所有维度参数从d10_*属性读取，不依赖self.num_heads
def d10_attention_forward(
    ...
    # KV cache - 兼容5.x Cache对象和旧版tuple
    ...
    # C5 head耦合
    coupling = self.d10_c5_coupling.to(attn_weights.device, attn_weights.dtype)
...
# Patch attention — 预存所有d10_*属性
for i, layer in enumerate(model.model.layers):
    attn = layer.self_attn
    # ★从config预存，不依赖self内部命名
...
print(f"  - φ-Residual: 通过attn/mlp输出α缩放")
...
# ============ Step 5: 五动诊断 ============
print("\n[5/5] 五动层诊断...")
# ★修复: tokenizer不一定返回position_ids，需要自动生成
...
try:
    ...
    try:
        layer_norms = []
        with torch.no_grad():
            x = model.model.embed_tokens(diag_tokens['input_ids'])
            for i, layer in enumerate(model.model.layers):
                x_out = layer(x, position_ids=diag_tokens.get('position_ids'))
