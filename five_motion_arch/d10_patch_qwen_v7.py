"""
D10五动管道架构改造脚本 v7
修复: 1) C5耦合维度 - 在Q的head维度操作而非attn_weights的key维度
      2) GQA expand在C5耦合之后
      3) 所有维度从d10_*属性读取
用法：python d10_patch_qwen_v7.py
"""
...
# C5 coupling matrix [num_heads, num_heads]
n_g = num_heads // 5
rem = num_heads % 5
...
# --- 重写Attention forward ---
# ★关键修复: C5耦合在Q上操作 [num_heads, head_dim], 不是在attn_weights上
def d10_attention_forward(
    ...
    # ★C5 head耦合 - 在Q的head维度上操作
    # q: [B, nh, S, hd], coupling: [nh, nh]
    # 混合不同head的Q向量: q_mixed[i] = sum_j coupling[i,j] * q[j]
...
print(f"  [3] Attention: C5耦合(Q-head混合)+RPB+温度+Z₂+α ✓")
