"""
D10五动管道架构改造脚本 v5
核心修复: 不替换layer.forward，用alpha缩放attn/mlp输出
- 保留HuggingFace原生generate/KV-cache/attention_mask管道
- D10改造仅在attn和mlp子模块级别生效
用法：python d10_patch_qwen_v5.py
"""
...
print(f"  层数: {num_layers}, heads: {num_heads}")
print(f"  hidden: {d_model}, d_ff: {d_ff}")
...
try:
    ...
    print(f"  ⚠ RoPE重建失败: {e}, 用手动φ²计算")

# 替换model级别的rotary_emb
...
# --- 重写Attention forward ---
# ★关键: 输出乘以alpha，让φ-Residual在子模块级别生效，不碰layer.forward
def d10_attention_forward(
    ...
    # RoPE - 优先用model传来的position_embeddings
    ...
    # C5 RPB + Z₂
    k_phase = self.layer_idx % 5
    g = self.layer_idx // 5 % 2
    ...
    # ★φ-Residual: 输出乘以alpha，等效于residual + alpha*attn_out
...
print(f"  [3] Attention: C5耦合+RPB+温度+Z₂+α缩放 ✓")
...
try:
    with torch.no_grad():
        ...
        for i, layer in enumerate(model.model.layers):
            # 直接调用原版layer.forward
            layer_out = layer(x.unsqueeze(0) if x.dim() == 2 else x,
                              position_ids=diag_in.position_ids,
                              use_cache=False)
            x = layer_out[0]
            k = i % 5
            g = i // 5 % 2
            layer_norms.append((i, k, g, x.norm().item()))

    print("\n  层级激活范数 (五动周期):")
    names = ['认','遇','落','裂','余']
    for i, (lidx, k, g, v) in enumerate(layer_norms):
        z2 = '⇄' if g == 1 else '→'
        print(f"  L{lidx:02d} [{names[k]}] {z2} norm={v:.4f}")
except Exception as e:
    print(f"  诊断失败: {e}")
    import traceback
    traceback.print_exc()
