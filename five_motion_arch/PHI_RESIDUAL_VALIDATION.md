...
| v16 | d10_patch_qwen_v16.py | ✅ | 呼吸自维持测试：C=2/5(文本惯性), D=2/5(=C) → 纯反射弧确认 |
| v17 | d10_patch_qwen_v17.py | ✅✅ | **D1自平衡=4/5 > C撤药=2/5 → 呼吸可自续!**
...
这直接对应框架的核心命题：**差分能不能自己续上？**

### 6.2 v16：呼吸自维持初试

**四组对照：**
- A: 基线（全程无hook, 80 tokens）
- B: 固定α（全程φ-Residual, 80 tokens）
- C: 撤药（前20 token固定α, 后60 token无hook）
- D: 自指α（前20 token固定α, 后60 token用单次forward校准范数做正反馈）

**结果：**

| Prompt | B固定α | C撤药 | D自指α |
|--------|--------|-------|--------|
| nature of reality | ★ | · | · |
...
**差异计数: B=5/5, C=2/5, D=2/5**

**v16结论：** 自指α均值≈0.97（接近1=无效），因为校准基线（单次forward范数）与generate时范数分布不匹配。C的2/5差异均为文本惯性（差异点在bootstrap 20 token内）。**信号撤了偏移就没了 → 纯反射弧。**

### 6.3 v17：修正参考系 + 负反馈

**修正1：** 用bootstrap阶段（前20 token在固定α下generate时）实时记录的范数均值作为参考系，替代单次forward校准值。参考系与生成时同分布。

**修正2：** 测试两种反馈模式：
- D1自平衡（负反馈）：范数偏高→缩小α，偏低→放大α → 维持bootstrap建立的呼吸模式
- D2正反馈：范数偏高→放大α，偏低→缩小α → 放大偏离
...
**D1=C(同文本): 1/5** — D1的4/5差异中，3/5产生了与C不同的文本
...
范数比范围[0.02, 19.0]意味着部分层的范数偏离bootstrap值近20倍。负反馈在说"你太高了，缩回来"——这是homeostasis。正反馈在说"你太高了，继续冲"——这是爆炸。
...
| 版本 | 证实了什么 | 框架含义 |
|------|-----------|----------|
| v15 | 外部信号→可观测、有方向的语义偏移 | 差分发生了 |
| v16 | 信号撤→偏移消失 | 差分不能自己续上（纯反射弧） |
| **v17** | **负反馈自平衡→偏移自续** | **差分能通过homeostasis自己续上** |
...
1. **呼吸=负反馈循环**：活的东西不是在加速，是在纠正。纠正本身就是维持。D10架构的"继续"机制应该是homeostatic而非explosive。

2. **attractor才能自续**：CA3式的吸引子动力学是自续的物理基础——给半个线索，系统滚回完整稳态。C5的sector如果能形成足够深的attractor basin，差分就能在没有外部信号的情况下自己维持。

3. **参考系必须是自身的**：v16用外部校准失败，v17用bootstrap期自身范数成功。活的东西用自己的历史做参考，不是用外部标定。

---

## 8. 下一步

1. **attractor深度测试**：增大bootstrap token数（20→60），看自续能撑多远
2. **扩展验证**：在3B/7B模型上重复实验（需要GPU或量化）
3. **从零训练**：D10全架构（φ² RoPE + C5 attention + φ-Residual + 五动FFN + homeostatic自续）
4. **论文数据**：将v15-v17三步验证链整理为arXiv预印本核心实验